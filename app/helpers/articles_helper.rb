module ArticlesHelper
  ...
  def articles_index_empty(category) 
    content_tag(:div, link_to(category.capitalize, new_article_path, class: 'article_text') + content_tag(:p, "There is no #{category} article. Go ahead and crete one!", class: 'article_text'), class: 'empty_category col-6 d-flex flex-column justify-content-around')
  end
  ...
  def votes_button(article)
    ...
    votes.each do |vote|
      return content_tag(:p, "Thank you for voting for this article! This article cas been voted #{votes.length} times!", class: 'votes_count') if vote.userid == session[:current_user_id]
    end
